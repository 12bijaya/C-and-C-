    ...
    void continue_execution(pid_t child_pid) {
        ...
        if (ptrace(PTRACE_CONT, child_pid, NULL, NULL) == -1) {
            perror("Error: ptrace CONT failed");
            return;
        }
        ...
        while (1) {
            ...
            if (WIFSTOPPED(status)) {
                if (WSTOPSIG(status) == SIGTRAP) {
                    ...
                    // RIP points to the instruction after the breakpoint (0xCC)
                    unsigned long breakpoint_address = regs.rip - 1;
                    printf("Breakpoint hit at 0x%llx\n", breakpoint_address);

                    // Call the function to restore the original instruction and adjust RIP
                    handle_breakpoint_hit(child_pid, breakpoint_address);

                    // Exit the loop after hitting a breakpoint
                    return;
                } else {
...
// Dummy function to resolve a function name to an address. You need to implement this.
unsigned long resolve_function_address(const char *function_name, const char *program_name) {
...
void set_breakpoint(pid_t child_pid, const char *target, const char *program_name) {
    ...
    // Try to read the memory at the address to validate it
    errno = 0; // Reset errno before the ptrace call
    long data = ptrace(PTRACE_PEEKTEXT, child_pid, (void *)address, NULL);
    if (data == -1 && errno != 0) {
    ...
    // Save the original byte and inject INT3
    ...
    *byte = 0xCC;
    ...
    void handle_breakpoint_hit(pid_t child_pid, unsigned long address) {
        for (int i = 0; i < breakpoint_count; i++) {
            if (breakpoints[i].address == address) {
                ...
                *byte = breakpoints[i].original_byte;
                ...
                // Adjust the instruction pointer to re-execute the original instruction
                struct user_regs_struct regs;
                if (ptrace(PTRACE_GETREGS, child_pid, NULL, &regs) == -1) {
                    perror("Error: ptrace GETREGS failed");
                    return;
                }
                regs.rip = address; // Move RIP back to the breakpoint address
                if (ptrace(PTRACE_SETREGS, child_pid, NULL, &regs) == -1) {
                    perror("Error: ptrace SETREGS failed");
                    return;
                }

                printf("Restored original instruction and adjusted RIP to 0x%lx\n", regs.rip);
                return;
            }
        }

        printf("No breakpoint found at address: 0x%lx\n", address);
    }
